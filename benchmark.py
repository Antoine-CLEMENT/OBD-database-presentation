...
engine = create_engine(f"postgresql://postgres:{POSTGRES_PASSWORD}@localhost:{PORT}/postgres", pool_size=50,
                       max_overflow=20)
...
def insertPsql():
    ...
    i = insert(netflix_movies).values(show_id='s' + str(idx), type='type', title='title', director='director',
                                      ...
                                      release_year=2014, rating='rating', duration='duration',
...
benchmark({'redis dir exists head': redis_director_exists_head,
...
def psql_indian_films():
    req = 'SELECT show_id FROM netflix_movies WHERE country = \'India\';'
    conn.execute(req)


benchmark({'redis indian films': redis_indian_films, 'psql indian films': psql_indian_films}, run_nb=10)
...
def redis_compute_mean():
    _sum = 0
    _count = 0
    for show_id in r.scan_iter():
        _count += 1
        _sum += int(r.hget(show_id, 'release_year'))
    return _sum / _count


def psql_compute_mean():
    req = 'SELECT avg(release_year) from netflix_movies;'
    conn.execute(req)
    return req
...
for j in range(10):
    print("STEP")
    median_redis, median_psql = benchmark_thread({'redis incr': (increment, 'r'), 'psql incr': (update, 'sql')}, engine,
                                                 1000, j * 5 + 1, False)
